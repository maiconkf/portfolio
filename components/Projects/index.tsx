        ...
        {arrProjects.map(({ project, description, link, img, width }, idx) => (
          <Grid key={idx} item xs={12} sm={4}>
            ...
            >
              ...
              <Typography sx={{ color: 'primary.contrastText', fontWeight: 500, lineHeight: 1.29 }} mt={1}>{project}</Typography>
              <Typography sx={{ color: 'primary.light', fontSize: 14, fontWeight: 500 }} mt={0.3}>{description}</Typography>
              <MuiBox sx={{ display: 'flex', alignItems: 'center', width: '100%' }} mt={1}>
                <MuiBox sx={{ backgroundColor: 'secondary.main', borderRadius: '100%', display: 'flex' }} p={0.5} mr={1}>
                  <LinkIcon sx={{ transform: 'rotate(-45deg)' }} fontSize="small" />
                </MuiBox>
                <Link sx={{
                  color: 'secondary.light', fontWeight: 500, whiteSpace: 'nowrap',
                  overflow: 'hidden',
                  textOverflow: 'ellipsis'
                }} href={link} target="_blank">{link}</Link>
              </MuiBox>
            </Box>
          </Grid>
        ))}
